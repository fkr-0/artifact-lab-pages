...
export const ARTIFACT_MOTION_PRESETS = Object.freeze({
  gentle: Object.freeze({ stiffness: 0.18, friction: 0.76, threshold: 10 }),
  native: Object.freeze({ stiffness: 0.24, friction: 0.70, threshold: 10 }),
  snappy: Object.freeze({ stiffness: 0.32, friction: 0.64, threshold: 8 }),
});

export function createDragInteraction(config) {
  return createArtifactMotion(config);
}

export function makeDropContext(extra) {
  return Object.freeze({ ...extra });
}

export function createArtifactMotion(userConfig) {
  const preset = userConfig?.preset ? (ARTIFACT_MOTION_PRESETS[userConfig.preset] || {}) : {};
  const cfg = {
    root: document,
  ...
  function onPointerMove(event) {
    ...
    cfg.onDragMove(makeDropContext({ event, sourceEl: drag.sourceEl, items: drag.items, targetEl: state.targetEl, velocity: velocity(drag), phase: "move" }));
  }
  ...
  function onPointerUp(event) {
    ...
    if (ok) {
      ...
      snapTo(drag, point, 'snap', () => {
        revealSources(drag);
        cfg.onDrop(makeDropContext({ event, sourceEl: drag.sourceEl, items: drag.items, targetEl, velocity: velocity(drag), phase: "drop" }));
        cfg.onDragEnd(makeDropContext({ event, sourceEl: drag.sourceEl, items: drag.items, targetEl, dropped: true, phase: "end" }));
      });
    } else {
      snapTo(drag, drag.origin, 'snap-back', () => {
        revealSources(drag);
        cfg.onCancel(makeDropContext({ event, sourceEl: drag.sourceEl, items: drag.items, phase: "cancel" }));
        cfg.onDragEnd(makeDropContext({ event, sourceEl: drag.sourceEl, items: drag.items, targetEl: null, dropped: false, phase: "end" }));
      });
    }
  ...
  function startDrag(event, drag) {
    ...
    startSpring(drag);
    ping('tap', 0);
    cfg.onDragStart(makeDropContext({ event, sourceEl: drag.sourceEl, items: drag.items, phase: "start" }));
  }
