...
export function createFloatingPanel({ title, url, iframeNode = null, dockLabel = '', onDock = null } = {}, runtime = globalThis) {
  ...
  // Count existing panels to cascade the initial position
  const existingPanels = documentRef.querySelectorAll?.('.floating') || [];
  const offset = existingPanels.length * 28;
  ...
  panel.style.left = `${Math.min(10 + offset, 30)}vw`;
  panel.style.top = `${Math.min(8 + Math.floor(offset / 2), 20)}vh`;
  panel.style.width = 'min(900px, 82vw)';
  panel.style.height = 'min(680px, 76vh)';
  panel.style.zIndex = String(++_floatingZBase);
  ...
  panel.addEventListener('pointerdown', () => { panel.style.zIndex = String(++_floatingZBase); }, true);
...
function makeFloatingPanelMovable(panel, runtime = globalThis) {
  ...
  const onPointerMove = (event) => {
    if (!drag) return;
    const vw = runtime.innerWidth || 800;
    const vh = runtime.innerHeight || 600;
    const pw = panel.offsetWidth || 0;
    const ph = panel.offsetHeight || 0;
    const nextLeft = Math.max(0, Math.min(vw - Math.max(80, pw), event.clientX - drag.offsetX));
    const nextTop = Math.max(0, Math.min(vh - Math.max(40, ph), event.clientY - drag.offsetY));
