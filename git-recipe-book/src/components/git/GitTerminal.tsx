...
export default function GitTerminal() {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      // The store owns command execution so every action happens exactly once.
  ...
  const handleKeyDown = useCallback(
    ...
    [input, navigateHistory, showCompletions, completions, completionIndex, getCompletions],
  )
  ...
  return (
    <div className="h-full flex flex-col bg-[#1a1b26] rounded-lg overflow-hidden border border-[#2a2b3d] shadow-lg">
      {/* Terminal Header */}
      ...
      <form onSubmit={handleSubmit} className="relative">
            ...
            >
              {completions.map((comp, i) => (
                <button
                  type="button"
                  key={comp}
                  className={`block w-full border-0 px-3 py-1 text-left text-xs font-mono cursor-pointer ${
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-t border-[#2a2b3d] bg-[#16171f]">
          <span className="text-[#bb9af7] font-bold text-sm">({branchName}) $</span>
