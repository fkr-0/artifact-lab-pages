...
import { helpProvider, useGitStore } from '@/stores/git-store'
import { BookMarked, BookOpen, ExternalLink, GitBranch, HelpCircle, Lightbulb, Search, Terminal, X } from 'lucide-react'
...
export default function GitHelpPanel() {
  ...
  const filteredHelp = searchQuery ? helpProvider.searchHelp(searchQuery) : allHelpTexts
  ...
  return (
    <Dialog open={helpPanelOpen} onOpenChange={(open) => setHelpPanel(open)} ariaLabel="Git Help & Reference">
      <div className="max-w-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border">
          ...
          <Button
            ...
            aria-label="Close Git help and reference"
          >
            <X className="w-4 h-4" />
          </Button>
        </div>
                ...
                <div key={ht.id} className="border rounded-lg p-3 space-y-2">
                  ...
                  {ht.examples.length > 0 && (
                    <div className="space-y-1.5">
                      ...
                      {ht.examples.map((ex) => (
                        <div key={`${ex.command}:${ex.description}`} className="bg-muted rounded-md p-2 space-y-1">
            ...
            <div className="space-y-3">
              {filteredGlossary.map((g) => (
                <div key={g.term} className="border rounded-lg p-3 space-y-1">
                  ...
                  {g.longDefinition && <p className="text-xs text-muted-foreground">{g.longDefinition}</p>}
