...
import { GitBranch as BranchIcon, Cloud, Globe, Star, Tag } from 'lucide-react'
      ...
      <div className="p-3 space-y-4">
        ...
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <h3 className="font-semibold text-xs uppercase tracking-wide">Branches</h3>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-colors ${
                      currentBranch === b.name ? 'bg-primary/10 border border-primary/20' : 'hover:bg-accent/50'
                    }`}
                  >
                    ...
                    {currentBranch === b.name && <Star className="w-3 h-3 text-amber-500 ml-auto shrink-0" />}
                    {!b.commitId && !tracking && (
                      <span className="text-[10px] text-muted-foreground ml-auto">(no commits)</span>
                    )}
                  </div>
                )
              })
        ...
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <h3 className="font-semibold text-xs uppercase tracking-wide">Remotes</h3>
          </div>
            ...
            {!hasRemotes && !remoteTrackingBranches.length ? (
              ...
              <>
                ...
                {remoteEntries.map(([remoteName, remote]) => (
                  <RemoteSection key={remoteName} remoteName={remoteName} remote={remote} />
                ))}
                  ...
                  <div className="space-y-1 ml-2">
                    {remoteTrackingBranches.map((b) => (
                      ...
                      >
                        ...
                        />
                        <span className="font-mono text-xs truncate text-cyan-600 dark:text-cyan-400">{b.name}</span>
        ...
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <h3 className="font-semibold text-xs uppercase tracking-wide">Tags</h3>
          </div>
          <div className="space-y-1">
        ...
        {gitState.stash.length > 0 && (
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <h3 className="font-semibold text-xs uppercase tracking-wide">Stash ({gitState.stash.length})</h3>
            </div>
            <div className="space-y-1">
              {gitState.stash.map((entry, i) => (
                <div key={entry.id} className="px-2 py-1.5 rounded-md text-xs hover:bg-accent/50 transition-colors">
                  <span className="text-muted-foreground">{`stash@{${i}}`}</span>: {entry.message}
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="border border-border/50 rounded-md overflow-hidden">
      <div className="flex items-center gap-2 px-2 py-1.5 bg-accent/30">
        ...
        <span className="text-[10px] text-muted-foreground truncate ml-auto">{remote.url}</span>
      </div>
