...
import { AlertCircle, FileCheck, FilePlus, FileText, GitBranch, GitCommitHorizontal } from 'lucide-react'
...
export default function StatusIndicator() {
  ...
  const tipCommit = HEAD.type === 'branch' ? commits[branches[HEAD.ref]?.commitId || ''] : commits[HEAD.commitId || '']
    ...
    <div className="absolute bottom-3 left-3 bg-card/90 backdrop-blur-sm border border-border rounded-lg p-2 text-[10px] space-y-1.5 shadow-lg">
      ...
      {tracking && (
        <div className="flex items-center gap-1 text-cyan-600 dark:text-cyan-400">
          ...
          <span>
            tracking {tracking.remote}/{tracking.remoteBranch?.split('/').pop()}
          </span>
        </div>
      )}
