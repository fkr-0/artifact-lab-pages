...
export function ThemeSwitcher({ currentTheme = 'dark', onThemeChange, variant = 'buttons' }: ThemeSwitcherProps) {
          ...
          <div className="absolute top-full left-0 mt-1 w-[280px] bg-popover border border-border rounded-lg shadow-lg z-50 p-1">
            {themes.map((theme) => (
              ...
              >
                ...
                {currentTheme === theme.id && <div className="w-2 h-2 rounded-full bg-primary" />}
              </button>
            ))}
          </div>
        )}
