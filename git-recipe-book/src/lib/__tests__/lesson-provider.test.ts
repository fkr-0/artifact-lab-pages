...
describe('LessonProvider', () => {
  ...
  describe('arePrerequisitesMet', () => {
    it('returns true when no prerequisites', () => {
      expect(provider.arePrerequisitesMet('orientation', new Set())).toBe(true)
    })

    it('requires orientation before command-heavy basics', () => {
      expect(provider.arePrerequisitesMet('basics', new Set())).toBe(false)
      expect(provider.arePrerequisitesMet('basics', new Set(['orientation']))).toBe(true)
    })
  ...
  describe('pedagogic contract', () => {
    it('explains Git and the subcommand before the first command is used', () => {
      ...
      if (!firstStep) throw new Error('Git Basics must contain an initial lesson step')

      expect(firstStep.toolIntroductions).toEqual(
        expect.arrayContaining([expect.objectContaining({ term: 'git' }), expect.objectContaining({ term: 'init' })]),
      )
      expect(firstStep.expectedOutcome).toContain('.git')
      expect(firstStep.checkpointQuestions?.length ?? 0).toBeGreaterThan(0)
    })
    ...
    it('has dedicated guided tutorial lessons for cherry-pick and tags', () => {
      ...
      expect(tags?.steps.map((step) => step.id)).toEqual(['tag-orient', 'tag-create', 'tag-list', 'tag-show'])

      expect(cherryPick?.steps.find((step) => step.id === 'cherry-pick-apply')?.safetyNote).toMatch(
        /duplicate|conflict|current branch/i,
      )
    ...
    it('marks dangerous history or file-state commands with safety notes', () => {
      ...
      for (const { lesson, step } of dangerousSteps) {
        expect(step.safetyNote, `${lesson.id}/${step.id} needs a safety note`).toMatch(
          /rewrite|discard|destroy|backup|shared|uncommitted/i,
        )
      }
    })
    ...
    it('keeps every bundled command step beginner-safe with tool explanations and outcomes', () => {
      ...
      for (const { lesson, step } of commandSteps) {
        expect(
          ...
          `${lesson.id}/${step.id} should explain its tools`,
        ...
        expect(
          ...
          `${lesson.id}/${step.id} should include a reflection checkpoint`,
        ).toBeGreaterThan(0)
      }
    })
  })
