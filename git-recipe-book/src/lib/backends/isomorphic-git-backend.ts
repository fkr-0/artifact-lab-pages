...
export class IsoGitBackend implements IGitBackend {
  ...
  createBranch(name: string): GitCommandResult {
    ...
    this.state.branches[name] = {
      name,
      commitId: this.state.branches[current]?.commitId || '',
  ...
  listBranches(): GitCommandResult {
    this.requireInit()
    const current = this.currentBranch()
    const lines = Object.keys(this.state.branches)
  ...
  merge(branchName: string): GitCommandResult {
    ...
    const id = generateId()
    this.state.commits[id] = {
      id,
      shortId: shortId(id),
      message: `Merge '${branchName}' into ${current}`,
      parentIds: [ourId, theirId],
      author: 'You <you@iso-git>',
      timestamp: now(),
      tree: mergedTree,
      branchLabel: current,
  ...
  rebase(targetBranch: string): GitCommandResult {
    return { success: false, output: '', error: 'Rebase not yet supported in isomorphic-git backend' }
  }
  log(count?: number): GitCommandResult {
    this.requireInit()
    const startId =
      this.state.HEAD.type === 'branch' ? this.state.branches[this.currentBranch()]?.commitId : this.state.HEAD.commitId
  ...
  status(): GitCommandResult {
    return { success: true, output: 'Status not fully implemented in isomorphic-git backend\n' }
  }
  diff(): GitCommandResult {
    return { success: true, output: 'Diff not fully implemented in isomorphic-git backend\n' }
  }
  ...
  listTags(): GitCommandResult {
    return { success: true, output: `${Object.keys(this.state.tags).join('\n')}\n` }
  }
  stash(message?: string): GitCommandResult {
    return { success: false, output: '', error: 'Stash not yet supported in isomorphic-git backend' }
  }
  stashPop(): GitCommandResult {
    return { success: false, output: '', error: 'Stash not yet supported' }
  }
  ...
  reset(target: string, mode?: 'soft' | 'mixed' | 'hard'): GitCommandResult {
    return { success: false, output: '', error: 'Reset not yet supported in isomorphic-git backend' }
  }
  cherryPick(commitHash: string): GitCommandResult {
    return { success: false, output: '', error: 'Cherry-pick not yet supported' }
  }
  ...
  fetch(remoteName: string): GitCommandResult {
    return { success: false, output: '', error: 'Fetch requires a real remote connection' }
  }
  pull(remoteName?: string, branchName?: string): GitCommandResult {
    return { success: false, output: '', error: 'Pull requires a real remote connection' }
  }
  push(remoteName?: string, branchName?: string): GitCommandResult {
    return { success: false, output: '', error: 'Push requires a real remote connection' }
  }
  ...
  execute(raw: string): GitCommandResult {
    ...
    return {
      success: false,
      output: '',
      error: 'Direct command execution not yet supported in isomorphic-git backend. Use the simulator for learning.',
    }
  }
  ...
  private currentBranch(): string {
    return this.state.HEAD.type === 'branch' ? this.state.HEAD.ref : '(detached)'
  }
  private findCommit(hash: string): GitCommit | null {
    if (this.state.commits[hash]) return this.state.commits[hash]
    for (const [, c] of Object.entries(this.state.commits)) {
      if (c.shortId === hash) return c
    }
    return null
  }
