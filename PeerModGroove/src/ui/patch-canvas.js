...
export class PatchCanvas {
  ...
  render() {
    ...
    data.nodes.forEach((n, i) => {
      if (!this.positions.has(n.id)) this.positions.set(n.id, { x: 24 + (i % 4) * 165, y: 24 + Math.floor(i / 4) * 110 });
    });
    ...
    this.root.querySelectorAll('.patch-port').forEach(port => port.onclick = e => this.clickPort(port, e.shiftKey));
  }
  ...
  edgePath(edge) {
    ...
    const x1 = a.x + 128, y1 = a.y + 62, x2 = b.x + 12, y2 = b.y + 62;
    const mid = (x1 + x2) / 2;
  ...
  bindDrag(el) {
    ...
    el.onpointermove = e => {
      ...
      this.positions.set(this.drag.id, { x: Math.max(0, this.drag.ox + e.clientX - this.drag.sx), y: Math.max(0, this.drag.oy + e.clientY - this.drag.sy) });
      this.render();
    };
    el.onpointerup = () => { this.drag = null; };
  }
  ...
  clickPort(port, disconnect = false) {
    ...
    if (!this.selectedPort) { this.selectedPort = current; port.classList.add('selected'); return; }
    ...
    if (from === to) { this.selectedPort = null; this.render(); return; }
    if (disconnect) this.graph.disconnect(from, to, 'audio'); else this.graph.connect(from, to, 'audio');
    this.selectedPort = null;
