...
export class PeernetStack extends EventTarget {
  ...
  available() {
    return Boolean(window.PeernetUserManager && window.PeernetSharedCore && window.PeernetSessionManager && window.PeernetStorageManager);
  }
  ...
  init(profile = {}) {
    ...
    this.core = this.core || new window.PeernetSharedCore({
    ...
    this.user = this.user || new window.PeernetUserManager({ storageKey: `${this.namespace}:identity` });
    ...
    this.sessions = this.sessions || new window.PeernetSessionManager({
    ...
    this.storage = this.storage || new window.PeernetStorageManager({
  ...
  bindEvents() {
    ...
    this._bound = true;
    this.core?.on('open', payload => this.emit('status', { text: `open:${payload.id}`, connected: true }));
    this.core?.on('hub:join', payload => this.emit('status', { text: `in session hub:${payload.id}`, connected: true }));
    this.core?.on('hub:ready', payload => this.emit('status', { text: `hosting:${payload.id}`, connected: true }));
