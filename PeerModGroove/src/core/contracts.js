...
export class ModuleBase extends EventTarget {
  constructor({ id = uid('mod'), title = 'Module', kind = 'utility', inputs = [], outputs = [] } = {}) {
    super();
    this.id = id;
    this.title = title;
    this.kind = kind;
  ...
  render() {}
  unmount() { if (this.root) this.root.innerHTML = ''; }
  async start(context) { this.ctx = context; }
  stop() {}
  receive(_packet, _inputId = 'in') {}
  connectAudio(_destination, _outputId = 'audio') {}
  disconnectAudio(_outputId = 'audio') {}
  serialize() { return { id: this.id, title: this.title, kind: this.kind }; }
  hydrate(_data) {}
}
