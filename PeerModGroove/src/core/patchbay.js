...
export class PatchBay extends EventTarget {
  ...
  removeModule(moduleId) {
    ...
    this.routes = this.routes.filter(r => r.from.moduleId !== moduleId && r.to.moduleId !== moduleId);
  ...
  route({ module, outputId, packet }) {
    this.dispatchEvent(new CustomEvent('packet', { detail: { from: module.id, outputId, packet } }));
