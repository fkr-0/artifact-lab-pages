...
export class BasicSynthModule extends ModuleBase {
  ...
  receive(packet) {
    ...
    if (packet.type === 'note-on') this.noteOn(packet.note, packet.velocity ?? 0.8);
  ...
  noteOn(note, velocity) {
    ...
    env.gain.exponentialRampToValueAtTime(Math.max(0.001, velocity), this.ctx.currentTime + 0.01);
    osc.connect(env);
  ...
  noteOff(note) {
    ...
    voice.env.gain.cancelScheduledValues(this.ctx.currentTime);
    voice.env.gain.setTargetAtTime(0.0001, this.ctx.currentTime, 0.05);
    voice.osc.stop(this.ctx.currentTime + 0.18);
  ...
  render() {
    ...
    `;
    this.root.querySelector('select').addEventListener('change', e => { this.waveform = e.target.value; });
  }
}
