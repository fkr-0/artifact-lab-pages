...
export class MultiSamplerModule extends ModuleBase {
  constructor(config = {}) {
    super({ id: config.id || uid('multisampler'), title: config.title || 'Slicing MultiSampler', kind: 'audio-source', inputs: [{ id: 'midi', type: PortType.MIDI }, { id: 'control', type: PortType.CONTROL }], outputs: [{ id: 'audio', type: PortType.AUDIO }] });
    this.output = null;
  ...
  async loadFile(file, rootNote = 'C4', minNote = 'C1', maxNote = 'C7') {
    ...
    this.zones.push({ rootNote, min: this.midi(minNote), max: this.midi(maxNote), buffer, name: file.name });
    this.fileName = file.name;
    this.render();
  }

  receive(packet) {
    if (packet.kind === PortType.MIDI && packet.type === 'note-on') this.play(packet.note, packet.velocity ?? .8, packetAudioTime(this.ctx, packet), packet.slice);
  }

  play(note = 'C4', velocity = .8, when = this.ctx?.currentTime || 0, slice = null) {
    ...
    amp.gain.setValueAtTime(Math.max(.001, velocity), when);
    src.connect(amp);
    amp.connect(this.output);
    if (slice !== null) {
      const dur = buffer.duration / this.sliceCount;
      src.start(when, Math.max(0, Number(slice) % this.sliceCount) * dur, dur * .98);
    } else src.start(when);
  }
  ...
  pitchRatio(note, root = 'C4') {
    const n = this.midi(note), r = this.midi(root);
  ...
  connectAudio(dest) { if (this.output && dest) this.output.connect(dest); }
  disconnectAudio() { try { this.output?.disconnect(); } catch (_) {} }
  ...
  render() {
    if (!this.root) return;
    const zoneRows = this.zones.map((z, i) => `<div class="zone-row" data-zone="${i}"><strong>${z.name}</strong><small>${this.noteName(z.min)}–${this.noteName(z.max)} root ${z.rootNote}</small><div class="waveform">${this.renderWaveform(z)}</div><label>min <input class="mini-input" data-min type="text" value="${this.noteName(z.min)}"></label><label>max <input class="mini-input" data-max type="text" value="${this.noteName(z.max)}"></label><label>root <input class="mini-input" data-root type="text" value="${z.rootNote}"></label></div>`).join('') || '<p class="microcopy">No samples loaded yet.</p>';
    ...
    this.root.querySelector('input[type=file]').addEventListener('change', e => [...e.target.files].forEach((file, i) => this.loadFile(file, 'C4', i ? 'C4' : 'C1', i ? 'C7' : 'B3')));
    this.root.querySelector('[data-slices]').addEventListener('input', e => this.sliceCount = Number(e.target.value) || 8);
    this.root.querySelectorAll('[data-zone]').forEach(row => {
      ...
      row.querySelector('[data-root]').onchange = e => z.rootNote = e.target.value || z.rootNote;
    });
  }
}
