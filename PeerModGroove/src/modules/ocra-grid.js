...
export class OcraGridModule extends ModuleBase {
  constructor(config = {}) {
    super({
      ...
      kind: 'midi-generator',
      inputs: [{ id: 'clock', type: PortType.CLOCK }, { id: 'control', type: PortType.CONTROL }],
      outputs: [{ id: 'midi', type: PortType.MIDI }, { id: 'audio', type: PortType.AUDIO }]
    });
    this.width = 8;
    this.height = 4;
    this.cursor = 0;
  ...
  receive(packet) {
    ...
    for (const note of this.notesAt(this.cursor)) {
      this.emitPacket(createMidiPacket('note-on', { note, velocity: 0.72, gate: 0.35, at: packet.at }), 'midi');
      this.emitPacket(createMidiPacket('note-off', { note, velocity: 0, at: packet.at + 0.35 }), 'midi');
  ...
  connectAudio(destination) { if (this.output && destination) this.output.connect(destination); }
  ...
  render() {
    ...
    this.root.querySelector('.inline-check input').addEventListener('change', e => { this.clickEnabled = e.target.checked; });
    this.renderGridCursor();
  }
