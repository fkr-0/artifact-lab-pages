...
export class PianoRollModule extends ModuleBase {
  ...
  receive(packet) {
    ...
    for (const note of this.pattern.filter(n => n.step === step)) {
      this.emitPacket(createMidiPacket('note-on', { note: note.note, velocity: note.velocity, gate: note.gate, at: packet.at }), 'midi');
      this.emitPacket(createMidiPacket('note-off', { note: note.note, velocity: 0, at: packet.at + note.gate }), 'midi');
    }
    this.root?.querySelectorAll('[data-step]').forEach(el => el.classList.toggle('active', Number(el.dataset.step) === step));
  }
