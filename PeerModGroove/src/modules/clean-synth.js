...
export class CleanSynthModule extends ModuleBase {
  constructor(config = {}) {
    super({
      ...
      inputs: [{ id: 'midi', type: PortType.MIDI }, { id: 'control', type: PortType.CONTROL }],
  ...
  receive(packet) {
    ...
    if (packet.kind === PortType.CONTROL && packet.type === 'param' && packet.target === 'cutoff') {
      ...
      if (this.filter) this.filter.frequency.setTargetAtTime(this.cutoff, this.ctx.currentTime, 0.02);
    }
  }
  ...
  render() {
    ...
    `;
    this.root.querySelector('[data-param="waveform"]').addEventListener('change', e => { this.waveform = e.target.value; });
    this.root.querySelector('[data-param="cutoff"]').addEventListener('input', e => {
      this.cutoff = Number(e.target.value);
      if (this.filter && this.ctx) this.filter.frequency.setTargetAtTime(this.cutoff, this.ctx.currentTime, 0.02);
    });
  }
}
