...
export class FieldRecorderModule extends ModuleBase {
  constructor(config = {}) {
    super({
      ...
      outputs: [{ id: 'audio', type: PortType.AUDIO }, { id: 'control', type: PortType.CONTROL }]
    });
    this.output = null;
    this.buffer = null;
  ...
  render() {
    ...
    `;
    this.root.querySelector('input').addEventListener('change', e => e.target.files[0] && this.loadFile(e.target.files[0]));
